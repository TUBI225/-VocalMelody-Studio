...
void testMetadataSerialization(TestContext& context) {
    ...
    const auto outcome = AudioFileImporter{}.import(wavPath);
    context.expect(outcome.has_value(), "corpus wav imports");
    if (!outcome.has_value()) {
        return;
    }

    const std::string json = audioMetadataToJson(outcome.value());
...
void testSaveMetadata(TestContext& context) {
    ...
    const auto outcome = AudioFileImporter{}.import(wavPath);
    context.expect(outcome.has_value(), "import works before saving metadata");
    if (!outcome.has_value()) {
        return;
    }

    const std::string metadataPath = vocalmelody::testing::tempFilePath("vms_meta.json");
    context.expect(saveAudioMetadata(outcome.value(), metadataPath),
                   "the metadata file is written");
...
void testSourceFileUnchanged(TestContext& context) {
    ...
    context.expect(outcome.has_value(), "import works on the corpus file");
...
void testSaveMetadataAccentedPath(TestContext& context) {
    ...
    const auto outcome = AudioFileImporter{}.import(wavPath);
    context.expect(outcome.has_value(), "a wav on a non-ASCII path imports successfully");
    if (!outcome.has_value()) {
        return;
    }
...
void testCorpusDiagnostics(TestContext& context) {
    ...
    const auto clippedOutcome = AudioFileImporter{}.import(clippedPath);
    context.expect(clippedOutcome.has_value() &&
                       clippedOutcome->analysis.clippingScore().value() > 0.0,
    ...
    const auto silentOutcome = AudioFileImporter{}.import(silentPath);
    context.expect(silentOutcome.has_value() &&
                       silentOutcome->analysis.voicePresenceScore().value() < 0.01,
