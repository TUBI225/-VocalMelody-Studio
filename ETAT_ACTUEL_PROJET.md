...
Le **socle de la phase 2 (Pitch Benchmark)** est implémenté, validé et **fusionné dans `main` (PR #3, merge commit `466f4cb`)** : structures `PitchFrame` / `PitchCandidate` / `PitchDistributionFrame` et types forts `MidiPitch` / `Cents` dans `src/common`, interface `IPitchEstimator` et baseline `AutocorrelationPitchEstimator` dans le nouveau module `src/pitch` (sans JUCE). La correction T-102.1 valide les bornes inclusives 80/2000 Hz et ferme `frequencyHzToMidi` sur les valeurs non finies. CTest 8/8 en Debug et Release, clang-format 35/35 et CI Windows verte (run `31893018165`). Le benchmark proprement dit (RMVPE/CREPE/pYIN/YIN, corpus vocal, mesures) reste à réaliser.
